	...
	for (size_t dataIdx = 0; dataIdx < dList->instructions.size(); dataIdx++)
	{
		...
		case G_MTX:
		{
			...
			else
			{
				...
				else
				{
					word0 = 0;
					word1 = 0;
					spdlog::error(StringHelper::Sprintf("dListDecl == nullptr! Addr = 0x%08X %s", GETSEGOFFSET(data), dbgName.c_str()));
				}
			}
		}
		break;
		...
		case G_SETTIMG:
		{
			...
			else
			{
				...
				else
				{
					word0 = 0;
					word1 = 0;
					spdlog::error("texDecl == nullptr! PTR = 0x{:08X} {}", texAddress, dbgName.c_str());
				}
			}
		}
		break;
		case G_VTX:
		{
			...
			else
			{
				...
				{
					spdlog::error(StringHelper::Sprintf("vtxDecl == nullptr! Addr = 0x%08X %s", GETSEGOFFSET(data), dbgName.c_str()));
				}
			}
		}
		break;
